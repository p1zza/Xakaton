...
struct DemoProduct {
    ...
    init (inTitle: String, inImage: Image) {
        title = inTitle
        image = inImage
    }
}
...
struct ProductsListView: View {
    ...
    @State var priceDistance = Int.random(in: 40...80)
    ...
    private func randomShops () {
        let coordinates = [
            ...
            CLLocationCoordinate2D(latitude: 55.6837004, longitude: 37.5224877)
        ]
                        ...
                        VStack (spacing: 16.0) {
                            ForEach (shops.sorted{
                                ...
                                HStack (spacing: 8.0) {
                                    ...
                                    Spacer()
                                    Button (action: {
                                        cart.addItem(item: ItemCart(inTitle: title, inLocation: item.location, inPrice: item.price, inImage: product.image))
                                        cart.calcTotals()
                                        showAlert.toggle()
                                    }) {
...
struct ProductPreviewView: View {
    ...
    @State var location: String
    @State var price: Int
    @State var product: DemoProduct
    @State var favorite: Bool = false
    @State var from: Bool = false
    @State var link: Int? = nil
    let locations = ["Москва, Вернадского 8", "Москва, Вернадского 19", "Москва, Преображенская 25"]
    
    var body: some View {
        ZStack {
            NavigationLink (destination: ProductView(title: product.title, location: location, price: price), tag: 1, selection: $link) {
                EmptyView()
            }
            NavigationLink (destination: ProductsListView(title: product.title, locations: locations, product: product), tag: 2, selection: $link) {
                EmptyView()
            }
            VStack (spacing: 16.0) {
                product.image
